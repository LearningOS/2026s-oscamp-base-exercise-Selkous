...
impl Sv39PageTable {
    ...
    pub fn extract_vpn(va: u64, level: usize) -> usize {
        ...
        let shift_amount = 12 + level * 9;

        // 右移将目标移动到最底端
        let shifted = va >> shift_amount;

        // 再用掩码提取低九位
        (shifted & 0x1FF) as usize
    }
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            ...
            let node = self.nodes.get(&curr_ppn).unwrap();
        ...
        let level_0_node = self.nodes.get_mut(&curr_ppn).unwrap();
        level_0_node.entries[vpn0] = (target_ppn << PPN_SHIFT) | flags;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        // 遍历到 level 0 时，PTE 必须是叶节点。TranslateResult
        let mut curr_ppn = self.root_ppn;
        ...
        for level in (0..=2).rev(){
            ...
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                ...
                let shift_amount = 12 + level * 9;
                let offset_mask = (1 << shift_amount) - 1;

                let base_pa = ppn << 12;
                let pa = (base_pa & !offset_mask) | (va & offset_mask);
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let mut curr_ppn = self.root_ppn;
            ...
            let node = self.nodes.get(&curr_ppn).unwrap();
            let pte = node.entries[vpn2];

            if pte & PTE_V == 0 {
                let new_ppn = self.alloc_node();

                let node = self.nodes.get_mut(&curr_ppn).unwrap();
                node.entries[vpn2] = (new_ppn << PPN_SHIFT) | PTE_V;
            ...
            let level_1_node = self.nodes.get_mut(&curr_ppn).unwrap();
            level_1_node.entries[vpn1] = (target_ppn << PPN_SHIFT) | flags;
    }
}
